...
export default function ConvitePage() {
  ...
  const years = useMemo(() => {
    const allYears = atuacoes.festivais_convite.map(a => a.data.match(/\d{4}/)?.[0] || "Desconhecido");
    return [...new Set(allYears)].filter(y => y !== "Desconhecido").sort((a, b) => b.localeCompare(a));
  }, []);

  const filteredAtuações = useMemo(() => {
    return atuacoes.festivais_convite.filter(a => {
  ...
  return (
    <>
      <section className="page-hero relative">
        <div className="absolute inset-0 bg-[url('/fotos/radio.jpg')] bg-cover bg-center opacity-40 pointer-events-none" />
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <h1 className="page-hero-title">
            Festivais a <span className="gold-accent">Convite</span>
          </h1>
          <p className="page-hero-subtitle">
            Eventos e festivais onde a Augustuna é convidada a atuar
          </p>
        </div>
      </section>
