...
export default function ConcursoPage() {
  ...
  const years = useMemo(() => {
    const allYears = atuacoes.festivais_concurso.map(a => a.data.match(/\d{4}/)?.[0] || "Desconhecido");
    return [...new Set(allYears)].filter(y => y !== "Desconhecido").sort((a, b) => b.localeCompare(a));
  }, []);

  const filteredAtuações = useMemo(() => {
    return atuacoes.festivais_concurso.filter(a => {
  ...
  return (
    <>
      <section className="page-hero relative">
        <div className="absolute inset-0 bg-[url('/fotos/cb.JPG')] bg-cover bg-center opacity-40 pointer-events-none" />
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <h1 className="page-hero-title">
            Festivais a <span className="gold-accent">Concurso</span>
          </h1>
          <p className="page-hero-subtitle">
            Competições onde a Augustuna leva o nome de Braga ao palco
          </p>
        </div>
      </section>
